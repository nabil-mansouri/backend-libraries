...
import com.nm.payment.operations.impl.TransactionChainCashImpl;
...
public class TestTransactionChainCash {

	//
	@Autowired
	private TransactionComputer computer;
	private TransactionChain chain = new TransactionChainCashImpl();
	...
	public void testShouldPushCash() throws Exception {
		...
		this.p.setPaymentType(PaymentType.Cash);
	...
	@Test()
	@Transactional
	public void testShouldRollbackCash() throws Exception {
		TransactionChainContext context = new TransactionChainContext();
		context.setFinderResult(new GraphFinderPath<Transaction>()
				.setFounded(new TransactionComposed().add(new TransactionState(TransactionStateType.Rollback))));
		context.getRequest().setAction(TransactionActionType.Push);
	...
	@Transactional
	public void testShouldNotPushCashWithCommit() throws Exception {
		TransactionChainContext context = new TransactionChainContext();
		context.setFinderResult(new GraphFinderPath<Transaction>()
				.setFounded(new TransactionComposed().add(new TransactionState(TransactionStateType.Rollback))));
