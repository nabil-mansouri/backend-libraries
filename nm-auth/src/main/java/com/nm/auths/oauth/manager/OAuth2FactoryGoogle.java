 ...
 *
 */
public class OAuth2FactoryGoogle extends OAuth2FactoryAbstract implements OAuth2Factory {

	public boolean accept(AuthenticationProvider provider) {
		return AuthenticationProviderDefault.Google.equals(provider);
	}

	public OAuth2RestTemplate get(AuthenticationProvider provider) throws Exception {
		return ApplicationUtils.getBean(ConfigurationAuthenticationsSecurity.GOOGLE_REST_TEMPLATE);
	}
	...
	public OAuth2RestTemplate build(AuthenticationProvider provider, List<String> scopes) throws Exception {
		OAuth2RestOperations original = ApplicationUtils
				.getBean(ConfigurationAuthenticationsSecurity.GOOGLE_REST_TEMPLATE);
