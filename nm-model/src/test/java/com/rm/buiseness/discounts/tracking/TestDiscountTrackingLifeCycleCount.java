...
public class TestDiscountTrackingLifeCycleCount {
	...
	public void testCountMax() throws NoDataFoundException {
		DiscountFormBean promo1 = discountBuilder.addTrackingLifeRules(DiscountTrackingLifeCycleRuleType.RelativeCount,
		...
		founded = trackProcessor.findInactive(context);
		Assert.assertEquals(0, founded.size());
		// used twice client1 = >not available
		soaDiscount.use(client1.getId(), promo1.getId());
		daoDiscountPlanning.flush();
		founded = trackProcessor.findActive(context);
		Assert.assertEquals(1, founded.size());
