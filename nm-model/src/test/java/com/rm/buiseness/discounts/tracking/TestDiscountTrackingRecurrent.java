...
public class TestDiscountTrackingRecurrent {
	...
	@Test
	@Transactional
	public void testPeriodic() throws NoDataFoundException {
		DiscountFormBean promo1 = discountBuilder.addTrackingLifeRules(DiscountTrackingLifeCycleRuleType.RecurrentPeriod,
				new DiscountLifeCycleTrackingRuleBean().setEnable(true).setPeriodic(true).setEveryPeriod(PeriodType.Day).setEveryPeriodNumber(4))
				.addTrackingLifeRules(DiscountTrackingLifeCycleRuleType.Duration,
		...
		// No tracking
		Collection<Long> founded = trackProcessor.findActive();
		Assert.assertEquals(0, founded.size());
		founded = trackProcessor.findInactive();
		Assert.assertEquals(0, founded.size());
		...
		founded = trackProcessor.findActive();
		...
		founded = trackProcessor.findInactive();
		Assert.assertEquals(1, founded.size());
		// 4 days limit => restart
		MutableDateTime from4 = new MutableDateTime();
		from4.addDays(-4);
		daoState.updateDate(d.getId(), from4.toDate());
		daoDiscountPlanning.flush();
		founded = trackProcessor.findRestart();
		Assert.assertEquals(1, founded.size());
	}
}
