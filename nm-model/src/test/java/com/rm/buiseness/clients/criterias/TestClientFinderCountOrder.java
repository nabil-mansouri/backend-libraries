...
public class TestClientFinderCountOrder {
	...
	@Test
	@Transactional
	public void testOrderedCountFromEmpty() throws Exception {
	...
	@Test
	@Transactional
	public void testOrderedCountFrom() throws Exception {
		// Not visited since 2 weeks
		ClientCriteriaRulesBean rules = new ClientCriteriaRulesBean().put(ClientCriteriaType.OrderCount,
	...
	@Test
	@Transactional
	public void testOrderedCountTo() throws Exception {
		// Not visited since 2 weeks
		ClientCriteriaRulesBean rules = new ClientCriteriaRulesBean().put(ClientCriteriaType.OrderCount,
				new ClientCriteriaRuleBean().setRangeTo(1d).setEnable(true).setHasRangeTo(true));
		rules = soaClient.save(rules);
		ClientCriterias c = daoClientCriterias.loadById(rules.getId());
		{
			// Never order so => nok
	...
	@Test
	@Transactional
	public void testOrderedCountBetween() throws Exception {
		// Not visited since 2 weeks
		ClientCriteriaRulesBean rules = new ClientCriteriaRulesBean().put(ClientCriteriaType.OrderCount,
				new ClientCriteriaRuleBean().setEnable(true).setRangeFrom(2d).setHasRangeFrom(true).setRangeTo(3d)
						.setHasRangeTo(true));
		rules = soaClient.save(rules);
		ClientCriterias c = daoClientCriterias.loadById(rules.getId());
		{
		...
		{
			// Prepare ORDERS for other client
			helper.addOrder(client, menuEt, test.getAllRestaurants().get(TestScenarios.RESTO1), 2, 1);
			daoOrderState.flush();
			Collection<Long> clientIds = clientFinderProcessor.find(c);
			Assert.assertEquals(0, clientIds.size());
		}
	...
	@Test
	@Transactional
	public void testOrderedCountBetweenDates() throws Exception {
		// Not visited since 2 weeks
		ClientCriteriaRulesBean rules = new ClientCriteriaRulesBean().put(ClientCriteriaType.OrderCount,
				new ClientCriteriaRuleBean().setEnable(true).setRangeFrom(2d).setHasRangeFrom(true)
