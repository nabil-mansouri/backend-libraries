...
import com.rm.dao.statistics.impl.MeasureProductQueryBuilder;
...
@Component(StatisticComputer.StatisticProductComputer)
public class StatisticProductComputerImpl implements StatisticComputer {
	@Autowired
	...
	public void compute(StatisticContextBean context) throws NoDimensionGeneratorException {
		...
		MeasureProductQueryBuilder query = MeasureProductQueryBuilder.get().withFilter(context.getFilter());
		if (query.hasLimit()) {
