 ...
 *
 */
public class VisitorCmdTranslate implements Visitor {
	private final TemplateParameterTranslation translate;

	public VisitorCmdTranslate(TemplateParameterTranslation font) {
		this.translate = font;
	}
	...
	public void startVisitRun(XWPFRun run, int index) {
		...
		if (text != null) {
			ReplaceResult res = translate.translate(text);
			if (res.getCount() > 0) {
				run.setText(res.getResult());
			}
		}
	}
