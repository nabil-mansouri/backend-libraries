...
public class LdapUtils {
	...
	public static LdapUser getLdapUser(String username, LdapContext context) {
		try {
			...
			if (username.contains("@")) {
				...
				String authenticatedLdapUser = (String) context.getEnvironment().get(Context.SECURITY_PRINCIPAL);
				if (authenticatedLdapUser.contains("@")) {
					domainName = authenticatedLdapUser.substring(authenticatedLdapUser.indexOf("@") + 1);
				}
			}
			...
			if (domainName != null) {
				...
				NamingEnumeration<SearchResult> answer = context.search(toDC(domainName), "(& (userPrincipalName="
	...
	public static LdapUser[] getLdapUsers(LdapContext context) throws NamingException {

		java.util.ArrayList<LdapUser> users = new java.util.ArrayList<LdapUser>();
		String authenticatedLdapUser = (String) context.getEnvironment().get(Context.SECURITY_PRINCIPAL);
		if (authenticatedLdapUser.contains("@")) {
			String domainName = authenticatedLdapUser.substring(authenticatedLdapUser.indexOf("@") + 1);
			SearchControls controls = new SearchControls();
			controls.setSearchScope(SUBTREE_SCOPE);
			controls.setReturningAttributes(userAttributes);
		...
		return users.toArray(new LdapUser[users.size()]);
	}
	...
	public static class LdapUser {
		...
		public String getLdapUserPrincipal() {
		...
		public void changePassword(String oldPass, String newPass, boolean trustAllCerts, LdapContext context)
			...
			try {
				...
				modificationItems[0] = new ModificationItem(DirContext.REMOVE_ATTRIBUTE, new BasicAttribute(
						"unicodePwd", getPassword(oldPass)));
				modificationItems[1] = new ModificationItem(DirContext.ADD_ATTRIBUTE, new BasicAttribute("unicodePwd",
						getPassword(newPass)));
