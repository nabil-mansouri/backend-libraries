...
import com.nm.carts.operations.CartOperationChainUnique;
...
public class TestChainUnique {
	...
	private CartOperationChainUnique chain = new CartOperationChainUnique();

	// TODO compute total and subtotal
	@Before
	...
	public void testShouldPushItemMultiple() throws Exception {
		...
		//
		Assert.assertEquals(1, dto.childrens().size());
		ext = (CartItemDtoExtImpl) dto.childrens().get(0);
		Assert.assertEquals(2, ext.getQuantity());
	}
	...
	public void testShouldAddItem() throws Exception {
		...
		Assert.assertEquals(1, dto.childrens().size());
		CartItemDtoExtImpl ext = (CartItemDtoExtImpl) dto.childrens().get(0);
		Assert.assertEquals(4, ext.getQuantity());
	}
	...
	public void testShouldMultiplyItemNumber() throws Exception {
		...
		//
		ext = (CartItemDtoExtImpl) dto.childrens().get(0);
		Assert.assertEquals(8, ext.getQuantity());
	}
